...
export default function TaskModal({ task, statuses, projectOptions, profilesAll, onClose, onSave, onDelete }) {
...
{" ".repeat(p.depth) + p.name}
            ...
            <div>
              ...
              <div className="mt-1 flex flex-wrap gap-1 mb-1">
                {assigneeList.map((name) => (
                                      <span
                                                      key={name}
                    className="inline-flex items-center gap-1 bg-blue-600/20 border border-blue-600 text-blue-300 text-xs px-2 py-0.5 rounded-full"
                  >
                    {name}
                    <button
                      type="button"
          ...
          <div className="flex items-center justify-between pt-2">
              ...
              {task && (
                  ...
                  type="button"
                  onClick={() => {
                                          if (confirm("Stergi acest task?")) onDelete(task);
                  }}
