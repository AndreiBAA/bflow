...
export default function GanttView({ projectOptions, tasks, statuses, onOpenTask }) {
  ...
  const months = [];
      ...
      while (cursor <= maxDate) {
              ...
              const offset = daysBetween(minDate, start) * dayWidth;
              const width = Math.max(1, daysBetween(start, end)) * dayWidth;
              months.push({ label: formatMonth(cursor), offset, width });
              cursor = next;
      }

  const todayOffset = today >= minDate && today <= maxDate ? daysBetween(minDate, today) * dayWidth : null;
  ...
  function zoomOut() {
          setDayWidth((w) => Math.max(MIN_DAY_WIDTH, w - 4));
  }
      function zoomIn() {
              setDayWidth((w) => Math.min(MAX_DAY_WIDTH, w + 4));
      }
      function zoomReset() {
  ...
  return (
          <div className="border border-gray-800 rounded-lg overflow-hidden">
            <div className="flex items-center justify-end gap-1.5 px-3 py-2 border-b border-gray-800 bg-[#12141c]">
              <span className="text-xs text-gray-500 mr-1">Latime luni</span>
            <button
              onClick={zoomOut}
              disabled={dayWidth <= MIN_DAY_WIDTH}
                            className="w-6 h-6 flex items-center justify-center rounded-md text-gray-400 hover:text-gray-200 hover:bg-[#181b24] disabled:opacity-30 disabled:hover:bg-transparent text-sm"
              title="Restrange"
            >
                            -
                  </button>
...
{months.map((m, i) => (
                    <div
                              key={i}
                   className="absolute top-0 h-full flex items-center px-2 text-xs text-gray-500 border-r border-gray-800 truncate"
...
{(() => {
                        ...
                        const offset = daysBetween(minDate, s) * dayWidth;
                        const width = Math.max(dayWidth - 4, (daysBetween(s, e) + 1) * dayWidth - 4);
