...
if (loading) return <div className="text-xs text-gray-500">Se incarca istoricul...</div>;
    if (entries.length === 0) return <div className="text-xs text-gray-600">Fara istoric inca.</div>;
...
{entries.map((entry) => {
  ...
  {entry.action === "archived" && <span className="text-orange-400">task arhivat</span>}
   {entry.action === "restored" && <span className="text-blue-400">task restaurat din arhiva</span>}
     </div>
      );
   })}
  </div>
